...
def generate(name, ics, gcs, render_fn, forced_output_dir=""):
    inits = render_fn(ics)
    goals = render_fn(gcs)
...
def puzzle(type='mnist', width=3, height=3, forced_output_dir=""):
    ...
    generate("-".join(["puzzle",type,str(width),str(height)]), ics, gcs, lambda configs: p.generate(np.array(configs),width,height), forced_output_dir)
